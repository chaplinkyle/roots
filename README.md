...
- **Live server actions.** A browser event invokes Java, rerenders the live tree,
  and returns a revisioned HTML patch. When one element-root component exactly
  contains the change, Roots transfers and morphs only that component.
- **Conventions over wiring.** Java packages become pages, layouts, dynamic routes,
  catch-all routes, and API handlers.
- **Tiny browser boundary.** The current unminified driver is small and
  uses browser APIs directly—no client library is embedded.
- **JDK-only core.** `roots-core` has no runtime dependency outside Java.
- **Standard container deployment.** The optional Jakarta Servlet 6.1 adapter
  runs the same runtime beneath an application-server context path with async SSE.
- **Shared JDBC state.** The optional JDBC adapter persists sessions, tagged
  application-cache values, and live-view owner leases without an ORM or runtime driver dependency.
...
Use `onChange` instead of `onInput` when an interaction should not rerender for
every edit, and measure high-frequency actions under expected load.
...
Map `dev.roots.servlet.RootsServlet` to `/*`, enable async support, and name the
application anchor with init parameters:

```xml
<servlet>
  <servlet-name>roots</servlet-name>
  <servlet-class>dev.roots.servlet.RootsServlet</servlet-class>
