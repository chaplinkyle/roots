...
final class RenderedTreePatch {
    ...
    static Patch between(RenderedTree previous, RenderedTree next) {
        ...
        return next.componentHtml().entrySet().stream()
                ...
                .filter(entry -> !entry.getValue().equals(previous.componentHtml().get(entry.getKey())))
    ...
    private static Patch componentPatch(RenderedTree previous, RenderedTree next, String id, String nextRegion) {
        ...
        var previousEnd = previousStart + previousRegion.length();
        var nextEnd = nextStart + nextRegion.length();
        var suffixLength = previous.html().length() - previousEnd;
        return suffixLength == next.html().length() - nextEnd
                && previous.html().regionMatches(previousEnd, next.html(), nextEnd, suffixLength)
                ? Patch.component(id, nextRegion) : null;
    }
