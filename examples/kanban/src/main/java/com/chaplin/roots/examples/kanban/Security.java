...
class Security {
    @Bean SecurityFilterChain filterChain(HttpSecurity http, Environment env) throws Exception {
        http.authorizeHttpRequests(rules -> rules
                ...
                .requestMatchers("/actuator/health/**", "/login.css").permitAll()
        ...
        switch (env.getProperty("kanban.auth", "local")) {
            case "local" -> {
                var encoder = new BCryptPasswordEncoder(12);
                var users = new InMemoryUserDetailsManager(List.of(
                        User.withUsername("admin").password("{bcrypt}" + encoder.encode(password(env, "ADMIN"))).roles("EDITOR").build()));
                http.userDetailsService(users);
                http.formLogin(login -> login.loginPage("/login").permitAll().defaultSuccessUrl("/app/", true));
            }
            case "oidc" -> {
                var registration = ClientRegistrations.fromIssuerLocation(Application.required(env, "KANBAN_OIDC_ISSUER"))
                        .registrationId("company").clientId(Application.required(env, "KANBAN_OIDC_CLIENT_ID"))
                        .clientSecret(Application.required(env, "KANBAN_OIDC_CLIENT_SECRET"))
                        .redirectUri(Application.required(env, "KANBAN_OIDC_REDIRECT_URI"))
            ...
            default -> throw new IllegalStateException("KANBAN_AUTH must be local or oidc");
        }
        return http.build();
    }
    ...
    private static String password(Environment env, String role) {
        var value = Application.required(env, "KANBAN_" + role + "_PASSWORD");
