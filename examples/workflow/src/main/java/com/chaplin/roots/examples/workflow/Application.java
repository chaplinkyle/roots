...
public class Application {
    ...
    static HikariDataSource pool(Environment env) {
        ...
        config.setJdbcUrl(required(env, "WORKFLOW_JDBC_URL"));
        config.setUsername(env.getProperty("WORKFLOW_JDBC_USER", ""));
        config.setPassword(env.getProperty("WORKFLOW_JDBC_PASSWORD", ""));
        config.setMaximumPoolSize(8);
        config.setMinimumIdle(1);
        config.setConnectionTimeout(2000);
        config.setValidationTimeout(1000);
        config.setPoolName("workflow");
    ...
    @Bean CustomerRepository customers(DataSource pool) { return new CustomerRepository(pool); }

    @Bean HealthIndicator workflowDatabase(DataSource pool) {
        return () -> {
            try (var connection = pool.getConnection(); var statement = connection.prepareStatement(
                    "SELECT version FROM workflow_customers WHERE customer_id = ?")) {
    ...
    @Bean RootsConfigCustomizer policies() {
        ...
        return config -> config
                ...
                .mapException(CustomerRepository.Missing.class, (request, failure) -> Response.text(404, "Record unavailable"))
